...
- **System tray (minimize-to-tray)**: clicar no X ou Alt+F4 esconde a janela para a bandeja do sistema em vez de encerrar o app. Ícone no tray com menu de contexto ("Mostrar" / "Sair"). Clique esquerdo no ícone restaura a janela. `CloseRequested` interceptado no Rust via `api.prevent_close()` + `window.hide()`. Feature `tray-icon` habilitada no Tauri, `TrayIconBuilder` configurado no `setup`. Multiplataforma (Windows, macOS, Linux).
- **Page Shell unificado**: `ScrollRegion` + `PageContainer` envolvem todas as rotas do módulo YouTube Music, eliminando wrappers de layout duplicados em 8 páginas. `ScrollRegion` é o único scroll container da tela, publicando o viewport via React context (`useScrollViewport`) para o virtualizer do `TrackTable` observar sem criar scroll próprio. `PageContainer` aplica `max-w-screen-xl + p-6 + gap-6 + @container/main` uniformemente. Playlist-page reestruturada: `CollectionHeader` virou irmão do `TrackTable` (antes era `headerContent` prop — vestígio do modelo antigo de scroll interno). `section-header` perdeu `px-2` que causava desalinhamento de 8px com grids irmãos. Spec: [page-shell-design.md](superpowers/specs/2026-04-11-page-shell-design.md). Referência: [smooth-wheel-scroll.md](reference/smooth-wheel-scroll.md).
- **Smooth wheel scroll**: hook `useSmoothWheel` intercepta eventos de `wheel` no viewport do `ScrollRegion` e anima `scrollTop` via `requestAnimationFrame` com curva ease-out (friction 0.95). Scrollbar drag, teclado e touch permanecem nativos. Implementação inspirada no gblazex/smoothscroll mas escrita do zero: per-element via ref, TypeScript, cleanup no unmount, clamped a `scrollHeight`. Chromium `SmoothScrolling` + `FractionalScrollOffsets` habilitados via `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` no Windows. CSS `scroll-behavior: smooth` removido (causava lag no thumb da scrollbar). Referência: [smooth-wheel-scroll.md](reference/smooth-wheel-scroll.md).
- **Iniciar rádio**: reprodução contínua por recomendações do YouTube Music, acionável a partir de música, playlist, álbum ou artista. Backend-first e demand-driven — busca faixas da API apenas quando o usuário rola a fila ou chega ao fim natural da reprodução (nunca carrega o pool inteiro de uma vez). Crate `ytmusic-api` ganhou `get_watch_playlist` com `WatchPlaylistRequest` (radio/shuffle/limit/continuation loop), parser de continuation tokens, e percent-encoding defensivo. `PlaybackQueue` ganhou `RadioState` com guard de in-flight, `append_radio_batch`, `truncate_after_current`. Novos comandos Tauri: `yt_radio_start`, `yt_radio_reroll`, `yt_radio_load_more`. Frontend: botão em artista, item nos menus de playlist/álbum/faixa/sidebar, badge "Rádio" no player bar, shuffle vira re-roll em modo rádio. Documentação: [youtube-music-radio-architecture.md](explanation/youtube-music-radio-architecture.md).
...
- **Tráfego de rede em idle no processo Rust** — em build de produção com a janela visível, sem música e sem interação, o Task Manager mostrava ~0.1 Mbps em bursts várias vezes por minuto no processo Rust enquanto o DevTools Network do WebView ficava vazio. Causa raiz: o `reqwest::Client` mantém pool de conexões idle por 90s (default) e os clients autenticados (`YtMusicClient::new`, `from_cookies`, download client) + handler `thumb://` em `lib.rs` mantinham conexões TCP/TLS persistentes vivas para a Google. O OS + stack TLS trocavam pacotes pequenos de keepalive periodicamente. Fix: configurar todos os builders de `reqwest::Client` com `pool_max_idle_per_host(0)` + `tcp_keepalive(None)` — fecha a conexão imediatamente após cada request. Trade-off aceito: cada request paga TLS handshake completo (~50–100ms), imperceptível para a UX. Detalhes: [network-connection-pool.md](explanation/network-connection-pool.md).
- **Limpeza completa de logs de debug artificiais** — removidos ~220 `console.log`/`console.warn` no frontend e ~370 `println!` no backend (Rust + crate `ytmusic-api`), mais o diretório inteiro `src/lib/debug/` e o módulo `services/perf.ts` (210 linhas, wrapper de IPC com timing). Preservados todos os `console.error` em catch blocks (51 ocorrências) e todos os `eprintln!` em error paths do Rust (30 ocorrências). Bug subjacente também corrigido: `startMemoryMonitor(5000)` rodava `setInterval` em produção sem cleanup. Reduz overhead de IPC, ruído de console e bundle size.
- **Animações CSS infinitas no shell consumindo CPU em idle** — `titlebar-glow` (`filter: drop-shadow` animado a 6s) e `animated-gradient-text` (`background-position` animado a 4s) rodavam infinitamente no titlebar e no TopBar do YouTube Music. Ambas forçavam repaint contínuo no main thread (não compositor-only) e contribuíam com ~7% de CPU em idle. Fix: animações removidas, `text-shadow` estático preservou o efeito visual sem custo de paint recorrente.
